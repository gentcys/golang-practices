...
const (
	...
	mmddyyyyForm = "01/02/2006" // we want the date input in this format
...
func doSlicesInterest(s1, s2 []string) bool {
	if s1 == nil || s2 == nil {
		return false
	}
	for _, str := range s1 {
...
func checkSibling(strs []string) error {
	...
	siblings := []string{"m", "f"}
	if siblings != nil && !doSlicesInterest(siblings, strs) {
		return errors.New("Please select a valid subling")
	}
